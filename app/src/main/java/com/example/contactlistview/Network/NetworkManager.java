...
public class NetworkManager {
    ...
    public void contactList(final Volleycallback callback) {
        ...
        if (isInternetConnection()) {
            ...
            RequestQueue queue = Volley.newRequestQueue(ContactsApp.getAppContext());
    ...
    public void profleDetail(final Volleycallback callback){
        if(isInternetConnection()) {
            RequestQueue queue = (RequestQueue) Volley.newRequestQueue(ContactsApp.getAppContext());
            final NetworkRequest stringRequest = new NetworkRequest(header, Request.Method.GET, url,
                    new Response.Listener<String>() {
                        @Override
                        public void onResponse(String response) {
                ...
                @Override
                public void onErrorResponse(VolleyError error) {
                    Log.e("catch", "error no response in profile ");
    ...
    public boolean isInternetConnection()
    {
        ...
        ConnectivityManager connectivityManager = (ConnectivityManager) ContactsApp.getAppContext().getSystemService(Context.CONNECTIVITY_SERVICE);
